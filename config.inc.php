<?php
    ...
    $servers = explode(',', getenv('POSTGRESQL_HOSTS'));
    for ($i = 0; $i < count($servers); $i++) {
        $hostvar = explode(':', $servers[$i]);
        $host = $hostvar[0];
        $port = 5432;
        if (count($hostvar) > 1)
            $port = $hostvar[1];
	    ...
	    $conf['servers'][$i]['desc'] = $host;

	    // Hostname or IP address for server.  Use '' for UNIX domain socket.
	    // use 'localhost' for TCP/IP connection on this computer
	    $conf['servers'][$i]['host'] = $host;

	    // Database port on server (5432 is the PostgreSQL default)
	    $conf['servers'][$i]['port'] = $port;
